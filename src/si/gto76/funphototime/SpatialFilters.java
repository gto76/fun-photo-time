	...
	private static int getSum(BufferedImage img, int x, int y, int[][] maska, int delitelj) {
		...
		for (int i = 0; i < size; i++) {
			for (int j = 0; j < size; j++) {
				...
				avgRed += maska[j][i] * Filters.getRed(rgb);
				avgGreen += maska[j][i] * Filters.getGreen(rgb);
				avgBlue += maska[j][i] * Filters.getBlue(rgb);
			}
		}
		rgb = Filters.setRed(rgb, avgRed / delitelj);
		rgb = Filters.setGreen(rgb, avgGreen / delitelj);
		rgb = Filters.setBlue(rgb, avgBlue / delitelj);
		return rgb;
	}
